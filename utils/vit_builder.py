...
class Prompt(nn.Module):
    ...
    def extra_repr(self) -> str:
        return f"length={self.prompt_len}, embed_dim={self.embed_dim}, prompt_init={self.prompt_init}, replace_token={self.replace_token}"


class MLPHead(nn.Module):
    def __init__(self, in_dim: int = 768, hidden_dims: list[int] = [2048, 768], out_dim: int = 100, act_layer=nn.GELU, norm_layer=None, drop=0.):
        super().__init__()
        layers = []
        dim_list = [in_dim] + hidden_dims + [out_dim]
        for l, (d_in, d_out) in enumerate(zip(dim_list[:-1], dim_list[1:])):
            if norm_layer is not None and l > 0:
                layers.append(norm_layer(d_in))
...
class Block(nn.Module):
    ...
    def forward(self, x):
        if self.prompt is not None:
            x_a = self.attn(self.norm1(self.prompt(x)))[:, :self.prompt.pre_tokens]
        else:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            prompt_len: Length of the prompt. 0 means not using prompt.
            prompt_init: 'uniform' or 'zero'
        """
        super().__init__()
        assert global_pool in ('', 'avg', 'token')
        if global_pool == 'avg':
            raise NotImplementedError("It may cause bugs when `global_pool='avg'` since the prompts have not been taken into account.")
        assert class_token or global_pool != 'token'
        assert prompt_args_dict['prompt_len'] >= 0
        ...
        self.prompt_len = prompt_args_dict['prompt_len']
        ...
        # self.prompt = prompt_fn(length=prompt_len, embed_dim=embed_dim, prompt_init=prompt_init, has_cls_token=True) if prompt_len > 0 else nn.Identity()
        ...
        if patch_drop_rate > 0:
            self.patch_drop = PatchDropout(
                patch_drop_rate,
                num_prefix_tokens=self.num_prefix_tokens + prompt_args_dict['prompt_len'],
            )
        else:
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                mlp_layer=mlp_layer,
                prompt=Prompt(prompt_len=_pd['prompt_len'], embed_dim=embed_dim, prompt_init=_pd['prompt_init']) if _pd['prompt_start_block'] <= i <= _pd['prompt_end_block'] and _pd['prompt_len'] > 0 else None
            )
    ...
    def get_intermediate_layers(
        ...
        outputs = [out[:, self.num_prefix_tokens + self.prompt_len:] for out in outputs]
    ...
    def forward_head(self, x, pre_logits: bool = False, last: bool = False) -> Tensor:
        if self.global_pool and not last:
            x = x[:, self.num_prefix_tokens + self.prompt_len:].mean(dim=1) if self.global_pool == 'avg' else x[:, 0]
        x = self.fc_norm(x)
        x = self.head_drop(x)
